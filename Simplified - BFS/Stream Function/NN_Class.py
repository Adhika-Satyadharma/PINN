...
class PINN_Channel_Flow():
    ...
    def loss_fn(self, x_bi, BCI_Data):
        ...
        loss += tf.reduce_mean(tf.square(BCI_Data[0][0] - D1_Pred[1]))  #u
        loss += tf.reduce_mean(tf.square(BCI_Data[0][1] + D1_Pred[0]))  #v

        # x_bi[1] = Neumann
        Temp_x = x_bi[1][:, 0:1]
        Temp_y = x_bi[1][:, 1:2]
        [D2_Pred] = self.CD.Get_Sim_Param(Temp_x, Temp_y, ["D2"])
        loss += tf.reduce_mean(tf.square(BCI_Data[1][0] - D2_Pred[1]))
        loss += tf.reduce_mean(tf.square(BCI_Data[1][1] + D2_Pred[0]))
        ...
        for i in range(len(self.Special_LF)):
            if self.Special_LF[i] == "max":
                ...
                LGFR_x = 1./3. - tf.reduce_mean(D1_Pred[1])
    ...
    def Update_Collocation_Points(self, X_r):
        # Used if collocation point change due to reshuffle, addition, deletion
        self.x = X_r[:, 0:1]
        self.y = X_r[:, 1:2]
...
class Print_Results():
    ...
    def Calc_Sim_Param(self):
        ...
        Temp_x = xy2[:, 0:1]
        Temp_y = xy2[:, 1:2]
        [Coor, Main_Var_NN, Derivative1, Derivative2, Temp_Residuals] = self.CD.Get_Sim_Param(Temp_x, Temp_y, "A")

        self.Main_Var_NN = [Derivative1[1], -1. * Derivative1[0]]
        self.Residuals = []
